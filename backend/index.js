...
app.post("/signup" , (req,res)=>{
    ...
    if ( !password ||  !passwordRegex.test(password)){
        return res.status(403).json({
            "error"  : "Password should  be 6 to 20 long with a numeric, 1 uppercase and 1 lowercase letters"
        })
    }
...
app.post("/signin" , async (req, res)=>{
   ...
   try{
        ...
        try {
            ...
            return res.status(500).json({err : "Error occured while login, please try again"}); 
        }
...
app.post("/google-auth" , async (req, res)=>{
  ...
  getAuth()
   ...
   .then(async (decodedUser)=>{
        ...
        let user =  await User.findOne({"email" :  email}).select("fullname username profile_img google_auth").then((u)=>{
            return u || null
        })
        .catch((error)=>{
            return res.status(500).json({"error" : error.message});
